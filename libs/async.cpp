...
namespace details {
  ...
  struct FileWriter {
    ...
    void writeToFile(const CommandBlock& block) {
      std::time_t time = std::chrono::system_clock::to_time_t(block.timestamp);
  ...
  struct Async : public Log {
    ...
    Context* createContext(size_t block_size) {
      std::shared_ptr<Context> context = std::make_shared<Context>(block_size);
   ...
   private:
    void comandParser(std::shared_ptr<Context> context, std::string_view str) {
